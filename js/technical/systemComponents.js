var systemComponents = {
	...
	'layer-tab': {
		...
		<div v-if="back" v-bind:style="{'position': 'fixed', 'width': '90px', 'height': buttonsHeightFunction(), 'background-color': '#0f0f0f', 'left': '0px', 'top': '0px', 'z-index': '99999', 'direction': 'rtl', 'max-height': '100vh', 'overflow-y': 'auto'}">
		<div style="width: 100%; height: 100%; direction: ltr;">
			...
			<button v-if="tmp.forest.layerShown" v-bind:class="back == 'big' ? 'other-back' : 'back'" v-on:click="showTab('forest', 'tree')" style="border-color: var(--wood); background-color: hsl(15, 60%, 10.6%);             margin-right: 20px; margin-left: 0; width: 50px; height: 50px; background-image: url(resources/forest-icon.webp); background-size: contain; top: 70px;" v-bind:style="{'box-shadow': player.tab=='forest'?'0 0 10px var(--wood)':''}"></button>
		</div>
		</div>
