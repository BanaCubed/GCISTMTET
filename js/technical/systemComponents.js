var systemComponents = {
	...
	'layer-tab': {
		...
		<div v-if="back" v-bind:style="{'position': 'fixed', 'width': '70px', 'height': buttonsHeightFunction(), 'background-color': '#0f0f0f', 'left': '0px', 'top': '0px', 'box-shadow': '0 0 10px 10px #0f0f0f', 'border-radius': '0 0 15px 0', 'z-index': '99999'}">
			<button v-if="tmp.field.layerShown" v-bind:class="back == 'big' ? 'other-back' : 'back'" v-on:click="showTab('options-tab', 'tree')" style="border-color: var(--ghop); background-color: hsl(0, 0%, 29.15%); width: 50px; height: 50px; background-image: url(resources/options-icon.webp);background-size: contain; top: 10px;" v-bind:style="{'box-shadow': player.tab=='options-tab'?'0 0 10px var(--ghop)':''}"></button>
			<button v-if="tmp.field.layerShown" v-bind:class="back == 'big' ? 'other-back' : 'back'" v-on:click="showTab('info-tab', 'tree')" style="border-color: var(--ghop); background-color: hsl(0, 0%, 29.15%);    width: 50px; height: 50px; background-image: url(resources/info-icon.webp);   background-size: contain; top: 20px;" v-bind:style="{'box-shadow': player.tab=='info-tab'?'0 0 10px var(--ghop)':''}"></button>
			<button v-if="tmp.field.layerShown" v-bind:class="back == 'big' ? 'other-back' : 'back'" v-on:click="showTab('field', 'tree')" style="border-color: var(--grass); background-color: hsl(105, 85%, 23.85%);   width: 50px; height: 50px; background-image: url(resources/field-icon.webp);  background-size: contain; top: 30px;" v-bind:style="{'box-shadow': player.tab=='field'?'0 0 10px var(--grass)':''}"></button>
			<button v-if="tmp.pres.layerShown" v-bind:class="back == 'big' ? 'other-back' : 'back'" v-on:click="showTab('pres', 'tree')" style="border-color: var(--pres); background-color: hsl(180, 34.45%, 31.8%);        width: 50px; height: 50px; background-image: url(resources/city-icon.webp);   background-size: contain; top: 40px;" v-bind:style="{'box-shadow': player.tab=='pres'?'0 0 10px var(--pres)':''}"></button>
			<button v-if="tmp.crys.layerShown" v-bind:class="back == 'big' ? 'other-back' : 'back'" v-on:click="showTab('crys', 'tree')" style="border-color: var(--crys); background-color: hsl(330, 53%, 37.1%);       width: 50px; height: 50px; background-image: url(resources/cave-icon.webp);   background-size: contain; top: 50px;" v-bind:style="{'box-shadow': player.tab=='crys'?'0 0 10px var(--crys)':''}"></button>
			<button v-if="tmp.hop.layerShown" v-bind:class="back == 'big' ? 'other-back' : 'back'" v-on:click="showTab('hop', 'tree')" style="border-color: var(--ghop); background-color: hsl(0, 0%, 29.15%);           width: 50px; height: 50px; background-image: url(resources/cult-icon.webp);   background-size: contain; top: 60px;" v-bind:style="{'box-shadow': player.tab=='hop'?'0 0 10px var(--ghop)':''}"></button>
		</div>
