var systemComponents = {
	...
	'layer-tab': {
		...
		<div v-if="back" v-bind:style="{'position': 'fixed', 'width': '70px', 'height': buttonsHeightFunction(), 'background-color': '#0f0f0f', 'left': '0px', 'top': '0px', 'box-shadow': '0 0 10px 10px #0f0f0f', 'border-radius': '0 0 15px 0', 'z-index': '99999'}">
			...
			<button v-if="tmp.field.layerShown" v-bind:class="back == 'big' ? 'other-back' : 'back'" v-on:click="showTab('info-tab', 'tree')" style="border-color: hsl(180, 100%, 50%); background-color: hsl(180, 100%, 26.5%);    width: 50px; height: 50px; background-image: url(resources/info-icon.webp);   background-size: contain; top: 20px;" v-bind:style="{'box-shadow': player.tab=='info-tab'?'0 0 10px cyan':''}"></button>
